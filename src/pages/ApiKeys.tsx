...
const CRED_ROOT = window.krypt.app.platform === 'darwin'
  ? '~/Library/Application Support/Krypt Trader'
  : '%APPDATA%/Krypt Trader';
...
export function ApiKeysPage() {
  ...
  return (
    <Page
      title="API Keys"
      subtitle={`Save Kalshi credentials for both Demo and Live accounts. Stored locally under ${CRED_ROOT}/credentials and never sent off-machine.`}
      actions={
      ...
      <Section title="Security notes">
        <Card>
          <ul className="list-disc space-y-1.5 pl-5 text-xs text-krypt-muted">
            <li>Keys are written per-env to <span className="font-mono text-white">{CRED_ROOT}/credentials/apikey.&lt;env&gt;.txt</span> with default user-only permissions.</li>
