...
def test_keychain_roundtrip():
    if not ka._keychain_available():
        pytest.skip("Keychain encryption is macOS-only")
    enc = ka._keychain_encrypt(b"super-secret")
    assert enc != b"super-secret"
    assert ka._keychain_decrypt(enc) == b"super-secret"
...
def test_credentials_encrypted_at_rest(tmp_path, monkeypatch):
    ...
    elif ka._keychain_available():
        assert raw.startswith(ka._KEYCHAIN_MARKER)
        assert b"my-api-key-uuid" not in raw
