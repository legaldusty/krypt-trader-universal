...
def resolve_db_path(arg_db: Optional[str]) -> Path:
    ...
    if appdata:
        candidates.append(Path(appdata) / "Krypt Trader")
    home = Path.home()
    if sys.platform == "darwin":
        candidates.append(home / "Library" / "Application Support" / "Krypt Trader")
    else:
        xdg = os.environ.get("XDG_CONFIG_HOME")
        candidates.append(Path(xdg) / "Krypt Trader" if xdg else home / ".config" / "Krypt Trader")
    for base in candidates:
        p = base / "data" / "krypt-trader.db"
        if p.exists():
            return p
