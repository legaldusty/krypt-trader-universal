...
_KEYCHAIN_MARKER = b"#KRYPT-KEYCHAIN-v1\n"
_KEYCHAIN_SERVICE = "cc.krypt.trader"
_KEYCHAIN_ACCOUNT = "credentials-key"
...
_cached_keychain_fernet: Optional[Fernet] = None
...
def _keychain_available() -> bool:
    if sys.platform != "darwin":
        return False
    try:
        return _keychain_fernet() is not None
    except Exception:
        return False


def _keychain_fernet() -> Optional[Fernet]:
    global _cached_keychain_fernet
    if _cached_keychain_fernet is not None:
        return _cached_keychain_fernet

    key = _keychain_find_password()
    if key is None:
        key = base64.urlsafe_b64encode(os.urandom(32)).decode("ascii")
        if not _keychain_add_password(key):
            return None
        stored = _keychain_find_password()
        if stored != key:
            return None
        key = stored

    try:
        _cached_keychain_fernet = Fernet(key.encode("ascii"))
    except Exception:
        return None
    return _cached_keychain_fernet


def _keychain_find_password() -> Optional[str]:
    try:
        out = subprocess.run(
...
def _keychain_add_password(secret: str) -> bool:
    try:
        out = subprocess.run(
...
def _keychain_encrypt(data: bytes) -> bytes:
    f = _keychain_fernet()
    if f is None:
        raise OSError("Keychain key unavailable")
    return f.encrypt(data)


def _keychain_decrypt(data: bytes) -> bytes:
    f = _keychain_fernet()
    if f is None:
        raise OSError("Keychain key unavailable")
...
def _secure_storage_available() -> bool:
    return _dpapi_available() or _keychain_available()
...
def _write_secret_bytes(path: Path, data: bytes) -> None:
    ...
    if _dpapi_available():
        ...
        try:
            tmp.write_bytes(_KEYCHAIN_MARKER + _keychain_encrypt(data))
            tmp.replace(path)
            _restrict_perms(path)
            return
        except Exception as e:
    ...
    if not _warned_plaintext:
        logger.warning("Credentials stored UNENCRYPTED (secure storage unavailable on this platform).")
...
def _read_secret_bytes(path: Path, upgrade: bool = True) -> bytes:
    ...
    if raw.startswith(_KEYCHAIN_MARKER):
        return _keychain_decrypt(raw[len(_KEYCHAIN_MARKER):])
    if upgrade and _secure_storage_available():
        try:
